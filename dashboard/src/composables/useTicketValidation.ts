...
const validateTicketResource = createResource({
	...
	onError: (error: any) => {
		...
		} else if (errorData.includes("This ticket is not valid for this event")) {
			showDebouncedToast("This ticket is not valid for this event.")
...
export function useTicketValidation(): TicketValidationState {
	...
	// Methods
	const validateTicket = (ticketId: string, selectedEvent?: string | null): void => {
		isProcessingTicket.value = true
		currentEvent.value = selectedEvent ?? null
		validateTicketResource.submit({
			ticket_id: ticketId,
			...(currentEvent.value ? { current_event: currentEvent.value } : {}),
		})
	}
	...
	const checkInTicket = (): void => {
		...
		checkInResource.submit({
			ticket_id: validationResult.value.ticket.id,
			...(currentEvent.value ? { current_event: currentEvent.value } : {}),
		})
	}
