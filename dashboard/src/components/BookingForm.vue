			...
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<div class="lg:col-span-2">
					...
					<AttendeeFormControl
						...
						:key="attendee.id"
						:ref="(el) => setAttendeeRef(el, index)"
						:attendee="attendee"
						:index="index"
					...
					<!-- Attendee count selector (only shown when event allows > 1) -->
					<div
						v-if="maxAttendeesPerBooking > 1"
						class="mt-6 bg-surface-white border border-outline-gray-3 rounded-xl p-4 shadow-sm"
					>
						<p class="text-xs font-medium text-ink-gray-6 mb-3">
							{{ __("Number of Attendees") }}
							<span class="text-ink-gray-4 font-normal">
								({{ __("max {0}", [maxAttendeesPerBooking]) }})
							</span>
						</p>
						<div class="flex flex-wrap gap-2">
							<button
								v-for="n in maxAttendeesPerBooking"
								:key="n"
								type="button"
								@click="selectedAttendeeCount = n"
								:class="[
									'w-10 h-10 rounded-lg text-sm font-semibold border transition-all',
									selectedAttendeeCount === n
										? 'bg-ink-gray-9 text-white border-ink-gray-9'
										: 'bg-surface-white text-ink-gray-7 border-outline-gray-3 hover:border-outline-gray-4',
								]"
							>
								{{ n }}
							</button>
...
function setAttendeeRef(el, index) {
	if (el) {
		attendeeFormRefs.value[index] = el;
	} else {
		delete attendeeFormRefs.value[index];
	}
}
...
const addAttendee = () => {
	if (attendees.value.length >= maxAttendeesPerBooking.value) {
		toast.warning(
			__("This event allows a maximum of {0} attendee(s) per booking.", [
				maxAttendeesPerBooking.value,
			])
		);
		return;
	}
...
// ── Attendee count selector state ──────────────────────────────────────────────

/** Maximum attendees allowed by the event (from eventDetails). */
const maxAttendeesPerBooking = computed(() =>
...
watch(
	...
	(max) => {
		if (selectedAttendeeCount.value > max) selectedAttendeeCount.value = max;
	},
	{ immediate: true }
);

// React to the user picking a different count — grow or shrink the array.
watch(selectedAttendeeCount, (newCount) => {
	const current = attendees.value.length;
	if (newCount > current) {
		// Add blank attendees up to newCount
		for (let i = current; i < newCount; i++) {
			attendees.value.push(createNewAttendee());
		}
	} else if (newCount < current) {
		// Trim excess attendees from the tail
...
const validateForm = () => {
	...
	// Validate phone numbers via intl-tel-input for each attendee
	attendees.value.forEach((_attendee, index) => {
		const formRef = attendeeFormRefs.value[index];
		if (formRef && typeof formRef.validatePhone === "function") {
			const phoneValid = formRef.validatePhone();
			if (!phoneValid) {
				errors.push(__("Valid phone number is required for Attendee #{0}", [index + 1]));
			}
		}
	});
