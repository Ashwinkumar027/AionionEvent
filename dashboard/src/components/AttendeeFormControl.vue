...
// Phone input refs – keyed by attendee index
const phoneRefs = ref({});

function setPhoneRef(el, index) {
	if (el) {
		phoneRefs.value[index] = el;
	} else {
		delete phoneRefs.value[index];
	}
}

/** Call this to validate the phone field for this attendee. Returns true if valid. */
function validatePhone() {
	const ref = phoneRefs.value[props.index];
	if (!ref) return true; // not mounted yet – skip
	return ref.validate();
}
