...
def process_booking(
	...
	# Enforce max attendees per booking limit
	max_allowed = int(event_doc.max_attendees_per_booking or 1)
	if len(attendees) > max_allowed:
		frappe.throw(
			_("This event allows a maximum of {0} attendee(s) per booking.").format(max_allowed)
		)
	if len(attendees) == 0:
		frappe.throw(_("At least one attendee is required."))

	# ── Client-type restriction ───────────────────────────────────────────────
	client_type = (event_doc.client_type_allowed or "").strip()
	if client_type:
		_validate_client_type(attendees, client_type)
...
def _validate_client_type(attendees: list[dict], client_type: str) -> None:
	"""
	Enforce client-type restrictions before a booking is created.

	client_type == "Existing Client"
	    Every attendee's email OR phone must exist in the Existing Client table.

	client_type == "New Client"
	    Every attendee's email OR phone must NOT exist in Existing Client AND
	    must have no prior Event Booking history (via Event Booking Attendee).
	"""
	EXISTING_CLIENT_MSG = _(
		"Only existing clients can book this event. "
		"Please contact your relationship manager."
	)
	NEW_CLIENT_MSG = _(
		"This event is only for new clients. "
		"Please contact your relationship manager."
	)
	...
	for attendee in attendees:
		...
		if not email and not phone:
			# Attendee has neither — let required-field validation catch it
			continue

		# ── Single OR query against tabExisting Client ────────────────────
		conditions = []
		values: dict = {}
		...
		is_existing = bool(
			frappe.db.sql(
				f"SELECT 1 FROM `tabExisting Client` WHERE {where_clause} LIMIT 1",
				values,
			)
		)
		...
		if client_type == "Existing Client":
			if not is_existing:
				frappe.throw(EXISTING_CLIENT_MSG)

		elif client_type == "New Client":
			if is_existing:
				frappe.throw(NEW_CLIENT_MSG)

			# Also block if the attendee has ANY prior booking history
			prior_conditions = []
			prior_values: dict = {}
			if email:
				prior_conditions.append("eba.email = %(email)s")
				prior_values["email"] = email
			if phone:
				prior_conditions.append("eba.phone = %(phone)s")
				prior_values["phone"] = phone

			prior_where = " OR ".join(prior_conditions)
			has_prior_booking = bool(
				frappe.db.sql(
					f"""
					SELECT 1
					FROM `tabEvent Booking Attendee` eba
					INNER JOIN `tabEvent Booking` eb ON eb.name = eba.parent
					WHERE ({prior_where})
					  AND eb.docstatus IN (0, 1)
					LIMIT 1
					""",
					prior_values,
				)
...
@frappe.whitelist()
def validate_ticket_for_checkin(ticket_id: str, current_event: str | None = None) -> dict:
	...
	if current_event and str(ticket_doc.event) != str(current_event):
		frappe.throw(_("This ticket is not valid for this event."))
...
@frappe.whitelist()
def checkin_ticket(ticket_id: str, current_event: str | None = None) -> dict:
	frappe.only_for("Frontdesk Manager", True)

	checkin_date = frappe.utils.today()
	validation_result = validate_ticket_for_checkin(ticket_id, current_event=current_event)
...
def _compute_forward_hash(
	merchant_key, txnid, amount, productinfo, firstname, email, merchant_salt
):
	"""SHA-512 forward hash: key|txnid|amount|productinfo|firstname|email|udf1|...|udf10|SALT"""
	# Fixed sequence with exactly 16 pipes (17 fields total)
	seq = f"{merchant_key}|{txnid}|{amount}|{productinfo}|{firstname}|{email}|||||||||||{merchant_salt}"
...
def _verify_payment_with_payu(merchant_key, merchant_salt, txnid, is_test):
	"""
	Call PayU's server-side Verify Payment API using Merchant Transaction ID (txnid).
	Returns the transaction dict from PayU on success, or None on failure.
	"""
	import requests

	verify_url = _PAYU_VERIFY_TEST if is_test else _PAYU_VERIFY_PROD

	# PayU verify hash: sha512(key|'verify_payment'|txnid|SALT)
	# Critical: var1 should be txnid for verify_payment command
	verify_hash_str = f"{merchant_key}|verify_payment|{txnid}|{merchant_salt}"
	verify_hash = hashlib.sha512(verify_hash_str.encode("utf-8")).hexdigest()
	...
	for i in range(3):
		try:
			...
			try:
				...
				frappe.log_error(f"PayU Verify API for {txnid} returned non-JSON response: {response.text[:200]}", "PayU Verify")
				return None
			...
			if i == 2:
				frappe.log_error(f"PayU Verify API error for {txnid}: {exc}", "PayU Verify")
				return None
			time.sleep(2)
	...
	# Response from verify_payment is keyed by txnid
	txn = transaction_details.get(str(txnid))
...
def get_payu_payment_data(booking_id: str, payment_gateway: str | None = None) -> dict:
	"""
	Generate (or retrieve from cache) all parameters required by PayU Bolt.
	...
	phone = (user_data.get("mobile_no") or attendee_phone or "").strip() or "9999999999"

	# Forward hash (Standard 16-pipe sequence):
	# key|txnid|amount|productinfo|firstname|email|udf1|udf2|udf3|udf4|udf5|udf6|udf7|udf8|udf9|udf10|salt
	# udf1-udf10 are empty for now
	hash_string = (
...
def confirm_payu_payment(txnid, status, mihpayid=None, payu_response=None) -> dict:
	...
	try:
		...
		# 4. Local Reverse Hash Construction (17-Field Sequence)
		# Format: [additionalCharges|]salt|status|udf10|...|udf1|email|firstname|productinfo|amount|txnid|key
		res_u = lambda f: res.get(f, "")
		status_v = res.get("status") or res.get("txnStatus") or ""
		
		hash_sequence = [
			merchant_salt,
			status_v,
			res_u("udf10"), res_u("udf9"), res_u("udf8"), res_u("udf7"), res_u("udf6"),
			res_u("udf5"), res_u("udf4"), res_u("udf3"), res_u("udf2"), res_u("udf1"),
			res.get("email"), res.get("firstname"), res.get("productinfo"),
			res.get("amount"), res.get("txnid"), merchant_key
		]
		hash_body = "|".join([str(x or "") for x in hash_sequence])

		additional_charges = res.get("additionalCharges")
		if additional_charges:
			reverse_str = f"{additional_charges}|{hash_body}"
		else:
			reverse_str = hash_body
		...
		if is_verified:
			...
			# Extra success log
			frappe.log_error(f"Payment SUCCESS: {booking_id}, txnid={txnid}", "PayU Final Success")
...
def _cleanup_payu_cache(booking_id, txnid):
	frappe.cache.delete_value(f"payu_txnid:{txnid}")
...
def payu_payment_callback(**kwargs):
	"""Server-to-server callback invoked by PayU."""
	...
	# Reverse verification (Standard 17-field sequence)
	res = data
	res_u = lambda f: res.get(f, "")
	status_v = res.get("status") or res.get("txnStatus") or ""
	
	hash_sequence = [
		merchant_salt, status_v,
		res_u("udf10"), res_u("udf9"), res_u("udf8"), res_u("udf7"), res_u("udf6"),
		res_u("udf5"), res_u("udf4"), res_u("udf3"), res_u("udf2"), res_u("udf1"),
		res.get("email"), res.get("firstname"), res.get("productinfo"),
		res.get("amount"), res.get("txnid"), merchant_key
...
def _mark_booking_paid(booking_id: str, txnid: str, mihpayid: str):
	"""Submit the booking and record payment. Idempotent."""
